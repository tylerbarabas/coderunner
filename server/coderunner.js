...
exports.getAnimsJson = function(req, res) {
  request.post(domain+'/anims-json?filtered=0&flat=1&tags=image',{},function(error,response,body){
...
exports.upload = function(req, res) {
    var appDir = path.dirname(require.main.filename),
        ...
        dest = appDir + '/public/modules/coderunner/img/';
    ...
    form.parse(req, function (err, fields, files){
	    ...
	    fs.rename(files.file[0].path,newPath);
            res.status(200).json({ 'filepath': newPath.split('/public')[1] });
        } else {
